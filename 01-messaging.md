...
The sender of `peer_storage`:
  - MUST support `option_want_storage`.
  - MAY send `peer_storage` to only peers who support `option_provide_storage`.
...
- If it does store the message:
  - MUST immediately respond with `your_peer_storage` containing `blob`
